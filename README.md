...
搜索会按局面自动选择阶段策略：开局依靠大师开局库缩小候选并采用较保守的混合评分；中局完整使用 NNUE 并进行主要的深度搜索；残局因剩余子力较少而提高枚举深度，同时把 NNUE 权重降到 20%。接口返回的 `phase` 为 `opening`、`middlegame` 或 `endgame`，便于记录和分别评测。阶段阈值与具体参数会通过固定局面集和成对对局继续校准。
